  ...
  stages {
    ...
    steps {
	 ...
	 stage('Deploy to Nexus') {
         steps {
             bat "mvn -DskipTests=true -Dmaven.skipTests=true -f pom.xml clean package -Dversion=${pom.version} deploy:deploy -DaltDeploymentRepository=localnexus::default::snapshots"
         }
      }	
